...
class cancelButton(tk.Button):
    ...
    def __init__(self, frameRef:tk.Frame):
        super().__init__(frameRef, text="Cancel", font=["Century Gothic", 20], command=lambda: frameRef.destroy())
...
class ShoppingFrame(tk.Frame):
    ...
    def SetupLayout(self):
        ...
        cancelButton(self).grid(row=0, column=5)


    def SubmitButtonClick(self):
        tk.Button(self, text="Reset", font=["Century Gothic", 20], command=lambda: ShoppingFrame(self.master,self)).grid(row=0, column=4)
        db = ImazonDatabase("./Imazon.db")
        ...
        for item in result:
            print(item[0])
            tk.Label(self,text = item[0], font=["Century Gothic", 20]).grid(row=result.index(item) + 2, column=0)
